...
namespace _1CRenameDesigner
{
    public partial class FormMe : Form
    {
        ...
        private void NotifyIconMe_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                MethodInfo mi = typeof(NotifyIcon).GetMethod("ShowContextMenu", BindingFlags.Instance | BindingFlags.NonPublic);    // - получить метод правой кнопки
        ...
        private void TimerMe_Tick(object sender, EventArgs e)
        {
            ...
            foreach (ManagementObject queryObj in searcher.Get())
            {
                ...
                {
                    if (MassCommand[i].IndexOf("/F") > 0) { CaptionAdd = MassCommand[i].Replace("/F", "") + "  "; break; }              // - имя базы для файловой версии
                    else if (MassCommand[i].IndexOf("/S") > 0) { CaptionAdd = MassCommand[i].Replace("/S", "") + "  "; break; }         // - имя базы для серверной версии
                    else if (MassCommand[i].IndexOf("/IBName") >= 0) { CaptionAdd = MassCommand[i].Replace("/IBName", "").Replace("\"", "") + "  "; break; }    // - имя базы из стартера 1С
                }
                //-------------------
                if (CaptionAdd == "║")                                  // --- Не нашли ключ для имени базы - запишем ошибку в лог (если разрешено)
                {
                    if (chkErrors.Checked) strResult += topResult.Replace("║", "Не найден ключ (/F,/S/IBName)") + strBottom + "\r\n- " + CaptionOld + "\r\n";
                }
                else if (CaptionOld.IndexOf(CaptionAdd) >= 0)           // --- существующий заголовок окна уже содержит имя базы - пропуск в лог (если разрешено)  
                {
                    if (chkSkip.Checked) strResult += topResult.Replace("║", "Пропуск (переименовано ранее)") + strBottom + "\r\n- " + CaptionOld + "\r\n";
                } 
                ...
                {   CaptionNew += CaptionAdd + CaptionOld;
                    ...
                    if (chkRename.Checked) strResult += topResult.Replace("║", "Замена") + strBottom + "\r\n-  С: " + CaptionOld + "\r\n- На: " + CaptionNew + "\r\n";
                }
            }
            //-------------------
